...
export function Attributes<T extends { [Index: string]: ValidAttributeTypes }>(Object: Instance) {
	return setmetatable({} as T & ((AttributeChanged: keyof T) => RBXScriptSignal), {
		__index: (_, Index) => {
...
export type Attributes<T extends {[Index: string]: ValidAttributeTypes}> = T & ((AttributeChanged: keyof T) => RBXScriptSignal);
...
export function GetAttribute<T extends ValidAttributeTypes>(Instance: Instance, Attribute: string, Value?: T): T extends undefined ? unknown | undefined : T {
	if (Value !== undefined && Instance.GetAttribute(Attribute) === undefined) {
